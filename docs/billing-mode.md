...
| **Test** | `"test"` | Test billing with Polar test mode keys. All gates/quotas work normally. UI shows amber "Test mode" badge. `BILLING_DEV_UNLOCK` still works as a convenience bypass in non-prod. |
...
| Polar webhook | `src/app/api/webhooks/polar/route.ts` | Returns `ignored` immediately |
...
- [`billing-polar.md`](billing-polar.md) — full Polar integration architecture
